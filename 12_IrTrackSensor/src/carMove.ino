...
void carMove(int leftSpeed, int rightSpeed)
{
  ...
  {
    //左电机前进
    digitalWrite(Left_motor_go, FORWARD); //左电机前进使能
  ...
  else if (leftSpeed < 0 && leftSpeed >= -255)
  {
    ...
    //左电机后退
    digitalWrite(Left_motor_go, REVERSE); //左电机前进禁止
    analogWrite(Left_motor_pwm, leftSpeed);
  }
  if (rightSpeed > 255)
    rightSpeed = 255;
  ...
  {
    //右电机前进
    digitalWrite(Right_motor_go, FORWARD); //右电机前进使能
  ...
  else if (rightSpeed < 0 && rightSpeed >= -255)
  {
    ...
    //右电机后退
    digitalWrite(Right_motor_go, REVERSE); //右电机前进禁止
    analogWrite(Right_motor_pwm, rightSpeed);
  }
}
