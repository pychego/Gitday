...
```mysql
show databases;			 # 显示当前用户的所有数据库
show tables;				 # 显示当前数据库所有table和view
desc v1; 						 # 显示视图结构
desc tb1;					   # 显示tb1的struct
```
...
```mysql
alter table tb1 modify name varchar(100);   # 修改已有列的数据类型
alter table tb1 add  briirth datetime;      # 在表格上追加列（append）
alter table tb1 add birth datetime first;   # 在第一列加入新列（insert）
...
```bash
create table 新表名 select * from 原表名；    # 完全copy table
create table 新表名 like 原表名；             # 只copy结构，不copy内容
...
```mysql
drop table tb1;        # 删除table
drop database db1;     # 删除数据库时里面内容同时清空
delete from tb1;       # 删除表中所有记录，打回empty set
...
普通的使用group by没有什么用，因为分组之后每组只会显示一条记录,和下面的设置条件分组显示对比，不`select *`，而是只显示有用的列
...
```
 ...
 必须指定连接键，相同时使用using函数，当连个表的连接列名不相同时，使用`on tb.empid=tb1.name`而且**select后面的列名必须指定表名。**

join默认是内连接，即只有两个表都有这个数据才会显示。`left join`是左外连接，即左侧表格的都会显示，不管右边表格有没有相应数据。与之相对的是`right join`。`full join`全外连接

### 自连接

将自身和自身连接在一起，联合之后会有`平方个`次数据。这和普通的连接不一样.因为**一般的join连接都指定了连接条件，自连接没有**。如果不指定普通连接的连接条件，结构就和自连接一样。
...
为了避免这种情况，在创建视图时可以指定`with check option`,即不接受与条件不匹配的记录。
...
```php
"" 和''都可以用来括住字符串，""中的变量会被解析，但是''不会。字符串之间用.连接
```
